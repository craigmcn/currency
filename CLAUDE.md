...
Config: `eslint.config.mjs` (ESLint 9 flat config). A legacy `.eslintrc` file is also present but should be deleted — see modernization tasks. Run `yarn lint` (read-only, fails on any error) or `yarn lint:fix`.
