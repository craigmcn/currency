...
interface IProps {
    ...
    options?: ISelectOption[] // TODO handle empty options better
...
const customStyles = {
  ...
  indicatorSeparator: (provided: object) => ({ ...provided, backgroundColor: grey600 }),
  placeholder: (provided: object) => ({ ...provided, color: grey600 }),
  singleValue: (provided: object) => ({ ...provided, overflow: "visible" })
}

export const SelectField: React.FC<TProps> = ({ id, label, selectedOption, options, handleChange }) => (
    <div className="form__group">
        <label className="form__label" htmlFor={ id }>{ label }</label>
        <Select id={ id } name={ id } options={ options } styles={ customStyles } onChange={ handleChange } value={ selectedOption } />
    </div>
)
